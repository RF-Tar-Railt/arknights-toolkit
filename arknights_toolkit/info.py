...
async def information(
        page: playwright.async_api.Page, name: str, upgraded: bool = False
):
    ...
    if (await but.count()) and upgraded:
        ...
        await page.locator(
            "text=个人工具 中文（中国大陆）‎创建账号登录 命名空间 页面讨论 变体 查看 阅读查看源代码查看历史 更多 搜索 搜索 前往"
        ).click()
...
async def talent(page: playwright.async_api.Page, name: str):
    ...
    block = page.locator(
        "//section[@class='mf-section-6 collapsible-block open-block']"
    )
...
async def document(page: playwright.async_api.Page, name: str):
    ...
    await page.click('[aria-label="用户导航"]')
    elem = page.locator(
        "//table[@class='wikitable mw-collapsible logo mw-made-collapsible']"
    )
    bounding = await elem.bounding_box()
    return await page.screenshot(full_page=True, clip=bounding, scale="device")
